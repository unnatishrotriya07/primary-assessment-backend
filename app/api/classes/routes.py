...
def create_class(class_in: ClassCreate, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    service = ClassService(db)
    return service.create_class(class_in)

@router.put("/{id}", response_model=ClassResponse)
def update_class(id: int, class_in: ClassUpdate, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    service = ClassService(db)
    return service.update_class(id, class_in)

@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_class(id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    service = ClassService(db)
