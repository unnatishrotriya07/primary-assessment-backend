    ...
    def import_students_excel(self, class_id: int, file_content: bytes, filename: str, tenant_id: Optional[str] = None) -> int:
        ...
        for data in parsed_students:
            ...
            if existing_student:
                raise HTTPException(
                    status_code=400,
                    detail=f"Student with Scholar Number '{scholar_number}' already exists in our database (Student Name: {existing_student.name}). Please use a unique scholar number."
                )
