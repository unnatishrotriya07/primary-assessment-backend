...
from .schemas import STTResponse, TTSRequest, TranscribeResponse, SpeakRequest
from .whisper_service import WhisperService, get_whisper_service
from .kokoro_service import KokoroService, get_kokoro_service
...
def save_uploaded_audio(content: bytes, interview_id: int, question_index: int) -> str:
    # Save folder: static/interviews/{interview_id}
    folder = os.path.join("static", "interviews", str(interview_id))
    os.makedirs(folder, exist_ok=True)
    
    # Save filename: q{question_index + 1}.wav
...
async def health_check(
    ...
    """
    Check if the voice services (Whisper and Kokoro) are available.
    """
...
async def speech_to_text(
    ...
    """
    Speech-to-Text transcription endpoint. Accepts audio file upload.
    """
...
async def transcribe_audio(
    ...
    """
    Local self-hosted Speech-to-Text endpoint using faster-whisper.
    """
    validate_audio_file(file.filename, file.content_type)
    content = await file.read()
...
async def text_to_speech(
    ...
    """
    Text-to-Speech audio generation endpoint. Returns MP3/WAV audio FileResponse.
    """
...
async def speak_text(
    ...
    """
    Synthesizes speech from text using local Kokoro API with caching and long-text chunking.
    """
